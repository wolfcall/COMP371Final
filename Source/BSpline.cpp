...
glm::vec3 BSpline::GetPosition(float t) //Need to use this - TODO
{
...
glm::vec3 BSpline::GetSpeed(float t) {
	...
	return vec3(GetWorldMatrix() * vec4(BSpline::GetSpeed(t - (int) t, mControlPoints[p1], mControlPoints[p2], mControlPoints[p3], mControlPoints[p4]), 1.0f));
}

glm::vec3 BSpline::GetTilt(float t) {
	...
	return vec3(GetWorldMatrix() * vec4(BSpline::GetTilt(t - (int) t, mControlPoints[p1], mControlPoints[p2], mControlPoints[p3], mControlPoints[p4]), 1.0f));
}
...
glm::vec3 BSpline::GetSpeed(float t, const vec3& p1, const vec3& p2, const vec3& p3, const vec3& p4)
{
    // Returns the position of the object on the spline
    // based on parameter t and the four control points p1, p2, p3, p4
    vec4 params(3.0f*t*t, 2.0f*t, 1, 0);
    mat4 coefficients(vec4(-1, 3, -3, 1), vec4(3, -6, 0, 4), vec4(-3, 3, 3, 1), vec4(1, 0, 0, 0));
    vec4 product = (1.0f / 6.0f) * params * coefficients;
...
glm::vec3 BSpline::GetTilt(float t, const vec3& p1, const vec3& p2, const vec3& p3, const vec3& p4)
{
    // Returns the position of the object on the spline
    // based on parameter t and the four control points p1, p2, p3, p4
